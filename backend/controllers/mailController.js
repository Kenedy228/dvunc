    ...
    try {
      ...
      if (request_firm) {
        ...
        fields = [
          ...
          "<p><b>Имя представителя</b></p> " +
            request_firm.firm_represantivename,
      ...
      let transporter = nodemailer.createTransport({
        ...
        port: 465,
        auth: {
          user: "testfortask@mail.ru",
          pass: "J47ffasJEmSJOQESxxKK",
        },
      });

      const mails = await Mail.findAll();
      const emails = mails.map((i) => i.mail_name);

      let result = await transporter.sendMail({
        from: '"ДВУНЦ (Заявки)" <testfortask@mail.ru>',
        to: emails,
