...
const AdminServicePage = observer(() => {
  ...
  useEffect(() => {
    try {
      fetchService(service_id).then((data) => [
        service1.setSelectedType(data.typeofservice),
        setName(data.service_title),
        setTitle(data.service_optionTitle),
        setDescr(data.service_description),
        setOption(data.service_option),
      ]);
  ...
  const delOption = async (id) => {
    try {
      ...
      if (isBoss) {
        await deleteOption(id).then((data) =>
          fetchService(service_id).then((data) => [
            setOption(data.service_option),
          ]),
        );
  ...
  const updOption = async () => {
    try {
      ...
      formData.append(
        ...
        service1.selectedType.typeofservice_id,
      );
      formData.append("service_option", JSON.stringify(option));
      await updateService(formData, service_id).then(
        alert("Данные услуги успешно обновлены"),
      );
    } catch (error) {
      alert(error.response.data.message);
    }
  ...
  return (
    <Container className="adminCourse mt-5 mb-5">
      <Row className="pb-5 justify-content-center">
        <Col md={6}>
            ...
            <Dropdown.Toggle variant="light">
              ...
              <Dropdown.Menu>
                {service1.types.map((type) => (
                  <Dropdown.Item
                    onClick={() => service1.setSelectedType(type)}
                    key={type.typeofservice_id}
                  >
                    {type.typeofservice_title}
                  </Dropdown.Item>
                ))}
              </Dropdown.Menu>
            </Dropdown.Toggle>
          ...
          <Form.Control
            ...
            className="mt-2"
          />
          <Button onClick={updOption} variant="dark" className="mt-3">
            Обновить
          </Button>
        </Col>
      </Row>
            ...
            <tbody>
              {option.map((option) => (
                <tr key={option.option_id}>
                  ...
                  <td>
                    <Button
                      onClick={() => delOption(option.option_id)}
                      variant="danger"
                    >
                      Удалить
                    </Button>
      ...
      <CreateOption
        ...
        onUpd={() => fetchOption(service_id).then((data) => [setOption(data)])}
      />
    </Container>
  );
});
