...
const AdminCoursePage = observer(() => {
  ...
  useEffect(() => {
    try {
      fetchCourseUpd(course_id).then((data) => [
        course.setSelectedType(data.type),
        course.setSelectedFormat(data.format),
        course.setSelectedDocument(data.document),
        setName(data.course_name),
        setDuration(data.course_duration),
        setPrice(data.course_price),
        setPdf(data.course_pdf),
      ]);
  ...
  const delShedule = async (id) => {
    try {
      ...
      if (isBoss) {
        await deleteShedule(id).then((data) =>
          fetchShedules(course_id).then((data) => shedule.setShedules(data)),
        );
      }
    } catch (error) {
  ...
  const delModule = async (id) => {
    try {
      ...
      if (isBoss) {
        await deleteModule(id).then((data) =>
          fetchModules(course_id).then((data) => module.setModules(data)),
        );
      }
    } catch (error) {
  ...
  const updCourse = () => {
    ...
    updateCourse(formData, course_id).then(
      alert("Данные курса успешно обновлены"),
    );
  };
    ...
    <Container className="adminCourse mt-5 mb-5">
      <Row className="pb-5 justify-content-center">
        <Col md={6}>
            ...
            <Dropdown.Toggle variant="light" className="w-5">
              ...
              <Dropdown.Menu>
                {course.formats.map((format) => (
                  <Dropdown.Item
                    onClick={() => course.setSelectedFormat(format)}
            ...
            <Dropdown.Toggle variant="light" className="w-5">
              ...
              <Dropdown.Menu>
                {course.documents.map((document) => (
                  <Dropdown.Item
                    onClick={() => course.setSelectedDocument(document)}
          ...
          <Row className="d-flex align-items-end">
            <Col md={1}>
              ...
              <a className="aContainer" href={baseURL + pdf} target="_blank">
                ...
                >
                  <path
                    d="M5.00008 3.33398C3.15841 3.33398 1.68342 4.82565 1.68342 6.66732L1.66675 33.334C1.66675 35.1757 3.14175 36.6673 4.98341 36.6673H25.0001C26.8417 36.6673 28.3334 35.1757 28.3334 33.334V13.334L18.3334 3.33398H5.00008ZM28.3334 13.334C22.8106 13.334 18.3334 8.85683 18.3334 3.33398L28.3334 13.334Z"
                    fill="#BFC3C8"
                  />
                  <path
                    d="M28.3334 13.334L18.3334 3.33398C18.3334 8.85683 22.8106 13.334 28.3334 13.334Z"
                    fill="#BFC3C8"
                  />
                  <path
                    d="M28.3333 13.334L18.3333 3.33398V11.334C18.3333 12.4386 19.2287 13.334 20.3333 13.334H28.3333Z"
                    fill="#79818C"
                  />
                </svg>
              </a>
          ...
          <Button onClick={updCourse} variant="dark" className="mt-3">
            Обновить
          </Button>
        </Col>
      </Row>
      <Row className="justify-content-center mt-5 pb-5">
        <Col md={6}>
            ...
            <tbody>
              {shedule.shedules.map((shedule) => (
                <tr key={shedule.shedule_id}>
                  ...
                  <td>
                    <Button
                      onClick={() => delShedule(shedule.shedule_id)}
                      variant="danger"
                    >
                      Удалить
                    </Button>
          ...
          <Button
            onClick={() => setSheduleVisible(true)}
            variant="dark"
            className="mb-5"
          >
            Добавить расписание
            ...
            <tbody>
              {module.modules.map((module) => (
                <tr key={module.module_id}>
                  ...
                  <td>
                    <Button
                      onClick={() => delModule(module.module_id)}
                      variant="danger"
                    >
                      Удалить
                    </Button>
      ...
      <CreateShedule
        show={sheduleVisible}
        onHide={() => setSheduleVisible(false)}
      />
      <CreateModule
        show={moduleVisible}
        onHide={() => setModuleVisible(false)}
      />
    </Container>
  );
});
