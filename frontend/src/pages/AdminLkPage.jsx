...
const AdminLkPage = observer(() => {
  ...
  useEffect(() => {
    try {
      fetchUserUpd().then((data) => [
        console.log(data),
        setLogin(data.user_login),
        setId(data.user_id),
        setRole(data.roleRoleId),
      ]);
    ...
    <Container className="adminLkPage mt-5 mb-5">
      <Row className="pb-5 justify-content-center">
        <Col md={6}>
          ...
          />
          <Button onClick={updateLogin} variant="dark" className="mt-2">
            Обновить
          </Button>
          ...
          <Form.Control value={pas} onChange={(e) => setPas(e.target.value)} />
          <Button onClick={updatePassword} variant="dark" className="mt-2">
            Обновить
          </Button>
            ...
            <tbody>
              {mail.map((i) => (
                <tr key={i.mail_id}>
                  <td>{i.mail_name}</td>
                  <td>
                    <Button onClick={() => delMail(i.mail_id)} variant="danger">
                      Удалить
                    </Button>
                  </td>
                </tr>
          ...
          <CreateMail
            show={mailVisible}
            onHide={() => setMailVisible(false)}
            onUpdate={() => fetchMails().then((data) => setMail(data))}
          />
          {role === 1 ? (
            <div className="users">
              ...
              <CreateUser
                ...
                onUpdate={() => fetchUsers().then((data) => setUsers(data))}
              />
            </div>
          ) : (
            <div></div>
