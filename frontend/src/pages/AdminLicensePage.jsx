...
const AdminLicensePage = observer(() => {
  ...
  const delLicense = async (id) => {
    try {
      ...
      if (isBoss) {
        await deleteLicense(id).then((data) =>
          fetchLicenses(null).then((data) => {
            license.setLicenses(data);
          }),
        );
  ...
  return (
    <Container className="adminLicensePage mt-5 mb-5">
        ...
        <tbody>
          {license.licenses.map((license) => (
            <tr key={license.license_id}>
              ...
              {license.license_file === null ? (
                ...
                <td>
                  <a
                    ...
                    href={baseURL + license.license_file}
                    target="_blank"
                  >
                    <svg
                      width="30"
              ...
              <td>
                <Button
                  onClick={() => delLicense(license.license_id)}
                  variant="danger"
                >
                  Удалить
                </Button>
      ...
      <Button onClick={updPdn} variant="dark" className="mt-3">
        Обновить
      </Button>
    </Container>
  );
